...
@section("title_content", "Keluhan Pasien")

@section("page_title" , "Keluhan Pasien")
                    ...
                    <table class="table table-striped table-bordered"  style="margin-top: 16px;" id="myTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td style="text-align: center">{{ $data->keluhan->status == "0"? '-' : 'Sudah' }}</td>
                                <td style="text-align: center">
                                    <button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#exampleModal-{{ $data->nik }}">
                                        Edit 
                                    </button>
                                    <a href="{{url("/dokter/keluhanpasien/" . $data->nik )}}" class="btn btn-success btn-sm">
                                        Detail
                                    </a>
                                    <form onsubmit="return confirm('Move data to trash?')" action="{{ url('/dokter/keluhanpasien/'.$data->id_antrian) }}" method="POST" style="display: inline">
                                        @method("DELETE")
                                        @csrf
                                            <input type="hidden" value="DELETE" name="_method">
                                            <input type="submit" value="Delete" class="btn btn-danger btn-sm">
                                        
                                        {{-- <button onclick="return confirm('Yakin Mau di Hapus ? ')" type="submit" class="btn btn-danger btn-sm" value="Delete">
                                            Hapus
                                        </button> --}}
                                    </form>
                                </td>
    ...
    <div class="modal fade" id="exampleModal-{{ $item->nik }}" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form action="{{ url('/dokter/keluhanpasien/'.$item->id_antrian) }}" method="POST" id="formulir-keluhanpasien-data">
                    @method("PUT")
                    @csrf
